...
binomial proportion][simple_nonconjugate].
...
n$). Thus, $x_i$ is a binary vector (a vector of zeros and ones) of length
...
group $k$ ($j = 1, \dots,R$, $k = 0, 1$). (Here, group 0 means forest
and group 1 means savanna.) Let $P_k$ denote the vector $(P_{k1},
\dots, P_{kR})$, and let $P$ denote all the (unknown) allele frequencies
$P = (P_0, P_1)$.

With this notation in place, we can state the problem, which is to
infer the unknowns $P$ and $Z$ from the genetic data $X$.
...
\prod_{k=1}^2 \prod_{j=1}^R p(P_{kj})$. This assumption could be
...
uniform prior distribution for $P_{kj}$, so $p(P_{kj}) = 1$.
...
[likelihood_ratio_simple_models]: https://pcarbo.github.io/fiveMinuteStats/.html
