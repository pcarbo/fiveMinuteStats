...
+ For $t = 1,2,\ldots$
    ...
    - Compute $A = \min \big\{1, \frac{\pi(y) \, Q(x_t \mid
      y)}{\pi(x_t) \, Q(y \mid x_t)} \big\}.$ $A$ is often called the
...
```{r trace-plot, fig.height=3, fig.width=5}
plot(x,xlab = "time",main = "values of x visited by the M-H algorithm")
```
...
```{r hist, fig.height=3, fig.width=3}
hist(x,xlim = c(0,10),probability = TRUE,main = "")
xx <- seq(0,10,length = 100)
lines(xx,target(xx),col = "red")
```

So we see that, indeed, the histogram of the values in $x$ indeed
provides a close fit to an exponential distribution.
...
$\pi(x) = c f(x)$ for some known $f$ with unknown constant $c$. This
