...
+ It is helpful if you are familiar with ridge regression, but this
  ...
  [Ryan Ribshirani's course notes][ryantibs-ridge].
...
stored as a vector $\mathbf{x}_i \in \mathbf{R}^p$. Typically one also
includes an intercept term, and we will ignore this detail here for
simplicity (noting that it is easy to add without fundamentally
changing the math). The main quantities of interest are the
coefficients $b_1, \ldots, b_p$, which are stored as a vector,
$\mathbf{b} \in \mathbf{R}^p$. For example we might want to compute
the posterior mean of each $b_j$.

Next we introduce the prior, which is that each of the coefficients is
normal with a mean of zero and the same variance (it doesn't have to
be the same, but here we'll assume that it is the same):
$$
b_j \sim N(0, \sigma_0^2).
$$

We will skip the derivation so that we can get more quickly to the
topic of interest, variational inference, but in the examples below we
will use the fact that the posterior distirbution of $\mathbf{b}$ is
the multivariate normal distribution with the following mean
$\bar{\mathbf{b}}$ and covariance $\mathbf{V}$:
$$
\bar{\mathbf{b}} = \mathbf{V} \mathbf{X}^T\mathbf{y}/\sigma^2,
\qquad
\mathbf{V} = \sigma^2(\mathbf{X}^T\mathbf{X} + \lambda \mathbf{I})^{-1},
$$
where $\lambda = \frac{\sigma^2}{\sigma_0^2}$, and $\mathbf{I}$ is the
$p \times p$ identity matrix.  Note that since everything is normal
in this model, the posterior mean is also the posterior mode.

Since posterior distribution is multivariate normal with analytic
expressions for the mean and covariance, clearly this is a case where
Bayesian computational techniques such as MCMC or variational
inference are not needed. However, for very large data sets (large $n$
and/or large $p$), computing the mean and covariance can be very
expensive—consider the effort involved in computing the matrix product
$$ and the matrix inverse in the expression for $\mathbf{V}$—so
actually these techniques could be useful even if on paper the
