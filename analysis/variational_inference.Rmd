...
$$
\mathrm{ELBO}(q) = 
\textstyle \int q(\mathbf{b}) \log p(\mathbf{b}) \, d\mathbf{b}
- \textstyle \int q(\mathbf{b}) \log q(\mathbf{b}) \, d\mathbf{b}.
$$

*Optional exercise: Rewrite these expressions using expectations
instead of integrals.*

TO DO: Make connection to CAVI.
