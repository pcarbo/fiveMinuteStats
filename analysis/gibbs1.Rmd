...
distribution $\Pr(X=x,Y=y) = p_{X,Y}(x,y)$ given by the following
table:
...
compute by the usual formula for conditional probability, $\Pr(A | B)
...
1. Simulate a new value of $X$ from $\Pr(X | Y=y)$ where $y$ is the
   current value of $Y$.

2. Simulate a new value of $Y$ from $\Pr(Y | X=x)$ where $x$ is the
   current value of $X$ (generated in 1.)
