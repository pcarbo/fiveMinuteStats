...
by 0 in what follows). Then we can rewrite this as
$$
...
The idea behind IS is that if $q$ is well chosen then the
approximation to $I$ will be better than the naive approximation.

# Examples

Suppose $X \sim N(0,1)$, and we want to compute $\Pr(X>z)$ for $z =
10$.  That is, $f(x) = I(x>10)$ and $p(x)= \phi(x)$ is the density of
...
many of the samples are $>z$, where $f$ is non-zero, and we hope to
get better precision. Of course, we could do this problem much better
ways... this is just a toy illustration of IS.
...
issue that can arise quite generally (not just for IS).
